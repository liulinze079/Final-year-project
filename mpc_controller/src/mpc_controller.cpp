                                           ...
                                           Eigen::Vector2d du_p)
{
  ...
  // Convert matrices to vectors for OSQP
  ...
  // Create vectors for the q vector and bounds
  ...
  // Problem dimensions
  c_int prob_m = 2 * dim_u * m_;  // Number of constraints
  c_int prob_n = dim_u * m_;      // Number of variables
  ...
  // Create CSC format matrices
  data->P = csc_matrix(prob_n, prob_n, P_data.size(), 
                      P_data.data(), P_indices.data(), P_indptr.data());
  data->q = q_data.data();
  data->A = csc_matrix(prob_m, prob_n, A_data.size(), 
  ...
  Eigen::Vector2d u;
  // Solve and check return status
  if (exitflag == 0) {
    osqp_solve(work);
    
    if (work->info->status_val == OSQP_SOLVED) {
      u = Eigen::Vector2d(work->solution->x[0] + du_p[0] + u_r[0], 
                        regularizeAngle(work->solution->x[1] + du_p[1] + u_r[1]));
      ROS_INFO("OSQP solved successfully");
    } else {
      ROS_WARN("OSQP solver status: %s", work->info->status);
      // Fall back to a simpler control strategy
      double dx = s_d[0] - s[0];
      double dy = s_d[1] - s[1];
      double dist = std::hypot(dx, dy);
      double angle_to_goal = std::atan2(dy, dx);
      double angle_diff = regularizeAngle(angle_to_goal - s[2]);
      
      // Simple proportional control
      double v = std::min(max_v_, 0.3 * dist);
      double w = 0.8 * angle_diff;
      
      ROS_INFO("Using fallback control: v=%.2f, w=%.2f", v, w);
      u = Eigen::Vector2d(v, w);
    }
  } else {
    ROS_ERROR("OSQP setup failed with code: %lld", exitflag);
